    ...
    <div className="">
      ...
      >
        <div className="relative mt-1">
          <div className="relative w-full cursor-default overflow-hidden rounded-lg dark:bg-slate-800 text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 focus-visible:ring-offset-2 focus-visible:ring-offset-teal-300 sm:text-sm">
            <Combobox.Input
              className="input py-3 pl-3 pr-10 focus:ring-0"
              placeholder={'Search facility type'}
              onChange={(event) => setQuery(event.target.value)}
            />
            <Combobox.Button className="absolute inset-y-0 right-0 flex items-center pr-2 dark:text-white">
              <ChevronDown />
            </Combobox.Button>
          </div>
          <Transition
            ...
            afterLeave={() => setQuery('')}
          >
            <Combobox.Options className="z-10 absolute mt-1 max-h-60 w-full bg-white dark:bg-slate-800 overflow-auto rounded-md py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
                  ...
                  <Combobox.Option
                    ...
                    className={({ active }) =>
                      `relative cursor-pointer select-none py-2 pl-10 pr-4 dark:text-white ${
                        active ? 'bg-green-500' : 'bg-white dark:bg-slate-800'
                      }`
                    }
                    value={facility}
                  >
